		...
		processingStepAddProperty: function () {
			...
			if ($prev_item.length > 0) {
				$prev_item.each(function () {
					$(this).find('.step').php('<i class="fal fa-check text-primary"></i>');

				});
			}
			var $tabs = $('a[data-toggle="pill"],a[data-toggle="tab"]');
			$tabs.on('show.bs.tab', function (e) {
				$(this).find('.number').php($(this).data('number'));
				var $prev_item = $(this).parent().prevAll();
				if ($prev_item.length > 0) {
					$prev_item.each(function () {
						$(this).find('.number').php('<i class="fal fa-check text-primary"></i>');
			...
			$('.next-button').on('click', function (e) {
				...
				$prev.find('.nav-link').addClass('active');
				$nav_link.find('.number').php('<i class="fal fa-check text-primary"></i>');
				$parent.find('.number').php('<i class="fal fa-check text-primary"></i>');
			});
		...
		currencySelect: function () {
			...
			$items.each(function () {
				$(this).on('click', function (e) {
					...
					$toggle_image.php($self.find('.image-flag').php());
